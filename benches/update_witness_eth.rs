...
fn benchmark_update_witnesses_batch(c: &mut Criterion) {
    ...
    for logn in 6..=12 {
        ...
        let (_s, vc_p) = test_parameter(12); // Pass logn directly to test_parameter
        let vc_c = VcContext::new(&vc_p, vc_p.logn);

        // Ensure vectors are consistent with n
        let v: Vec<Fr> = (1..=m).map(|x| Fr::from(x as u64)).collect();
        let gq = vc_c.build_commitment(&v).1; // This line will now have consistent input

        let alpha: Vec<usize> = (0..n).collect();
        let beta: Vec<usize> = (0..m).collect();
        let delta_value: Vec<Fr> = (1..=beta.len()).map(|x| Fr::from(x as u64)).collect();

        group.bench_function(format!("update_witnesses_batch_eth n = {}", n), |b| {
            b.iter(|| {
    ...
    for logn in 6..=12 {
        ...
        group.bench_function(format!("update_witness_eth n = {}", n), |b| {
            b.iter(|| {
